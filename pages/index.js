    ...
    <>
      ...
      <form className={`mt-10 border-4 border-gray-500 flex flex-col justify-center rounded-md px-8 py-8 w-full sm:w-2/3 md:w-2/3 lg:w-1/2 xl:w-1/3 2xl:w-1/4 ${addButtonClicked ? "hidden" : ""}`}>
        ...
        <div className="flex flex-col hidden">
          ...
          <input id="password" value={password} onChange={e => setPassword(e.target.value)} type="text" className="font-poppins text-gray-700 border-4 border-gray-700 focus:shadow-lg focus:border-gray-800 rounded-xl outline-none px-3 py-2 text-xl font-bold" required />
        </div>
        <p className={`font-medium ${createButtonClicked ? "hidden" : ""}`}>
          You can&apos;t find a workout activity in the list?
        <button type="button" onClick={() => setCreateButtonClicked(true)} className={`font-bold underline`}>Create it here.</button>
        </p>
      ...
      </form>
      {
        addButtonClicked &&
        <>
        <hr className="h-2 w-2/3 my-24" />
        <div className="mt-24">
          <p className="font-medium font-poppins text-2xl">You did <span className="text-blue-600 font-bold">{reps} {workoutActivity}!</span></p>
        </div>
        <button type="button" onClick={() => setAddButtonClicked(false)} className="mt-12 rounded-xl px-5 py-3 text-xl font-bold bg-gray-700 text-gray-50">Add new activity</button>
        </>
      }
